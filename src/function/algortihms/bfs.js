...
function bfsSegment(box, rows, cols, startPos, endPos, offset = 0) {
  const directions = [
    [-1, 0],
    [1, 0],
    [0, -1],
...
export default function generateBfs(box, rows, cols) {
  let start = null;
  let end = null;
  const bombSquad = [];
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (box[r][c] === 2) start = [r, c];
      if (box[r][c] === 3) end = [r, c];
      if (box[r][c] === 4) bombSquad.push([r, c]);
    }
  }
  if (!start || !end) return { newGrid: box, animation: [] };
  const typeGrid = box.map((row) => row.slice());
  ...
  for (let target of targets) {
    const { visitedOrder, path, distToTarget } = bfsSegment(
      box,
      rows,
      cols,
      currentStart,
  ...
  for (let v of totalVisitedOrder) {
    ...
    if (dist > currentMaxDist) {
      // Darken previous levels
      for (let pv of visitedCells) {
        const pr = pv.r;
        const pc = pv.c;
        if (cellShade[pr][pc] < 3) {
          cellShade[pr][pc]++;
  ...
  // Path animation with moving arrow and yellow
  const arrowValues = {
    right: 13,
    left: 14,
    up: 15,
