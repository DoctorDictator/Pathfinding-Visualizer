...
function dijkstraSegment(box, rows, cols, startPos, endPos, offset = 0) {
  const directions = [
    [-1, 0],
    [1, 0],
    [0, -1],
...
export default function generateDijkstra(box, rows, cols) {
  let start = null;
  let end = null;
  const bombSquad = [];
  ...
  for (let target of targets) {
    const { visitedOrder, path, distToTarget } = dijkstraSegment(
      box,
      rows,
      cols,
      currentStart,
