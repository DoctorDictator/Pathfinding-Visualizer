...
function convergentSwarmSegment(box, rows, cols, startPos, endPos, offset = 0) {
  const directions = [
    [-1, 0],
    [1, 0],
    [0, -1],
  ...
  pq.push([sr, sc], offset + 2 * manhattan(startPos, endPos));
  ...
  while (!pq.isEmpty()) {
    ...
    for (let [dr, dc] of directions) {
      ...
      if (
        ...
        const newDist = distMap[r][c] + getCost(getOriginalType(box[nr][nc]));
        if (newDist < distMap[nr][nc]) {
          distMap[nr][nc] = newDist;
          parent[nr][nc] = [r, c];
          pq.push([nr, nc], newDist + 2 * manhattan([nr, nc], endPos));
        }
      }
    }
  }
...
export default function generateConvergentSwarm(box, rows, cols) {
  let start = null;
  let end = null;
  const bombSquad = [];
  ...
  for (let target of targets) {
    const { visitedOrder, path, distToTarget } = convergentSwarmSegment(
      box,
      rows,
      cols,
      currentStart,
