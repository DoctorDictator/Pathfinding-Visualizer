...
function greedyBfsSegment(box, rows, cols, startPos, endPos, offset = 0) {
  const directions = [
    [-1, 0],
    [1, 0],
    [0, -1],
  ...
  while (openSet.length > 0) {
    openSet.sort((a, b) => manhattan(a, endPos) - manhattan(b, endPos));
    ...
    for (let [dr, dc] of directions) {
      ...
      if (
        ...
        const tentG = distMap[r][c] + getCost(getOriginalType(box[nr][nc]));
        if (tentG < distMap[nr][nc]) {
          distMap[nr][nc] = tentG;
          parent[nr][nc] = [r, c];
          openSet.push([nr, nc]);
        }
...
export default function generateGreedyBfs(box, rows, cols) {
  let start = null;
  let end = null;
  const bombSquad = [];
  ...
  for (let target of targets) {
    const { visitedOrder, path, distToTarget } = greedyBfsSegment(
      box,
      rows,
      cols,
      currentStart,
