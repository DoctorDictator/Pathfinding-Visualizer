...
function swarmSegment(box, rows, cols, startPos, endPos, offset = 0) {
  const directions = [
    [-1, 0],
    [1, 0],
    [0, -1],
  ...
  pq.push([sr, sc], offset + 0.5 * manhattan(startPos, endPos));
    ...
    for (let [dr, dc] of directions) {
      ...
      if (
        ...
        if (newDist < distMap[nr][nc]) {
          ...
          parent[nr][nc] = [r, c];
          pq.push([nr, nc], newDist + 0.5 * manhattan([nr, nc], endPos));
        }
      }
    }
  }
...
export default function generateSwarm(box, rows, cols) {
  let start = null;
  let end = null;
  const bombSquad = [];
  ...
  for (let target of targets) {
    const { visitedOrder, path, distToTarget } = swarmSegment(
      box,
      rows,
      cols,
      currentStart,
